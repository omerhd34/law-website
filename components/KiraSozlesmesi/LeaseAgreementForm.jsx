...
import {
 ...
 mergeFormState,
 textareaClass,
} from "./leaseAgreementFormConfig";
import { leaseAgreementSteps } from "./leaseAgreementFormSteps";
import { validateAll, validateStep } from "./leaseAgreementFormValidation";
import { LeaseAgreementPreviewSummary } from "./LeaseAgreementPreviewSummary";
import {
 LeaseStepDuration,
 LeaseStepExtra,
 LeaseStepGuarantor,
...
export default function LeaseAgreementForm() {
 ...
 const downloadPdf = async () => {
  ...
  try {
   ...
   a.href = url;
   a.download = "kira-sozlesmesi.pdf";
   a.click();
 ...
 const rentIncreaseSummary = useMemo(() => {
  const t = form.annualRentIncreaseNote?.trim();
  return t || "—";
 }, [form.annualRentIncreaseNote]);

 const currentStepMeta = leaseAgreementSteps.find((x) => x.id === step);
      ...
      <div className="flex items-start gap-3">
       ...
       <div className="min-w-0 space-y-1">
        ...
        <CardDescription className="text-pretty leading-relaxed">
         PDF dosyanız indirildi. Özeti aşağıda görebilirsiniz; gerekirse tekrar
         indirebilirsiniz.
        </CardDescription>
       </div>
      </div>
  ...
  <div className="mx-auto w-full max-w-6xl space-y-8 pb-10">
   <div className="space-y-2 text-center sm:text-left">
    ...
    <p className="max-w-4xl text-pretty text-sm text-muted-foreground md:text-[0.95rem]">
     Sekiz adımda taşınmazdan önizlemeye tüm bilgileri doldurun. Taslak metin
     avukat onayından sonra kesinleştirilebilir.
    </p>
   </div>
   ...
   <div className="rounded-2xl border border-border/70 bg-linear-to-b from-card to-muted/25 p-3 shadow-sm ring-1 ring-foreground/4 sm:p-4">
    ...
    >
     <div
      ...
      style={{
       width: `${(step / leaseAgreementSteps.length) * 100}%`,
      }}
     />
    </div>
    <div
    ...
    >
     {leaseAgreementSteps.map(({ id, title }) => {
      ...
      return (
       ...
       >
        ...
        <span className="text-left font-medium leading-tight tracking-tight">{title}</span>
       </button>
      );
     })}
    </div>
   ...
   <form
    className="lease-agreement-form"
    onSubmit={(e) => {
     e.preventDefault();
    }}
   >
    ...
    >
     <CardHeader className="border-b border-border/60 pb-4">
      ...
      <CardDescription>
       {step === 1 &&
        "Kiraya Veren (Ev Sahibi) tarafının kimlik ve iletişim bilgileri."}
       {step === 2 && "Kiracı tarafının bilgileri."}
       {step === 3 &&
        "Kiralanan gayrimenkulün bilgileri; adres parça parça girilir."}
       {step === 4 && "Sözleşme başlangıç ve bitiş tarihleri."}
       {step === 5 && "Kira bedeli ve ödeme koşulları."}
       {step === 6 && "Özel koşullar ve ek maddeler."}
       {step === 7 && "Müşterek müteselsil kefil bilgileri."}
       {step === 8 &&
     ...
     <CardContent className="space-y-6 pt-6">
      ...
      {step === 4 && (
       <LeaseStepDuration form={form} update={update} inputClass={inputClass} />
      )}
      {step === 5 && (
       <LeaseStepRent form={form} update={update} inputClass={inputClass} />
      )}
      {step === 6 && (
       <LeaseStepExtra
        form={form}
